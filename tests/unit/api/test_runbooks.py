"""Tests for the runbooks API handler."""
...
_HANDLER_PATH = (
    Path(__file__).resolve().parents[3] / "api" / "runbooks" / "handler.py"
)
_spec = importlib.util.spec_from_file_location("runbooks_handler", _HANDLER_PATH)
...
def test_list_runbooks(mock_execute):
...
@patch.object(handler, "_execute")
def test_delete_admin_allowed(mock_execute):
...
def test_delete_viewer_blocked():
    res = handler.lambda_handler(
        _event("DELETE", path_params={"id": "3"}, admin=False), None
    )
...
def test_delete_no_bearer_prefix_denied():
    ...
    raw_token = _jwt_with_groups(["dbops-admin"])  # valid admin token, no "Bearer "
    res = handler.lambda_handler(
...
def test_delete_viewer_group_denied():
    res = handler.lambda_handler(
        _raw_event("DELETE", f"Bearer {_jwt_with_groups(['dbops-viewer'])}",
                   path_params={"id": "3"}),
        None,
    )
    assert res["statusCode"] == 403
...
def test_delete_analyst_group_denied():
    # Unknown/other group without dbops-admin → deny (closes unknown-group hole).
    res = handler.lambda_handler(
        _raw_event("DELETE", f"Bearer {_jwt_with_groups(['dbops-analyst'])}",
                   path_params={"id": "3"}),
        None,
    )
    assert res["statusCode"] == 403
...
@patch.object(handler, "_execute")
def test_delete_no_groups_claim_is_admin(mock_execute):
    # Valid token with NO cognito:groups claim = one-admin dev fallback → admin.
    mock_execute.return_value = [{"id": 3}]
    res = handler.lambda_handler(
        _raw_event("DELETE", f"Bearer {_jwt_with_groups(None)}",
                   path_params={"id": "3"}),
        None,
    )
    assert res["statusCode"] != 403
...
@patch.object(handler, "_execute")
def test_delete_admin_group_allowed(mock_execute):
    mock_execute.return_value = [{"id": 3}]
    res = handler.lambda_handler(
        _raw_event("DELETE", f"Bearer {_jwt_with_groups(['dbops-admin'])}",
                   path_params={"id": "3"}),
        None,
    )
    assert res["statusCode"] != 403
