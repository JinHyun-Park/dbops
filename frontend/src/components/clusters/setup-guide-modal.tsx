...
export function SetupGuideModal({ open, onClose, clusterId, region }: Props) {
  ...
  const sqlPg = `${t("-- 1) DBOps 전용 read-only 롤 생성")}
${t("CREATE ROLE dbops_readonly LOGIN PASSWORD '<생성한-비밀번호-붙여넣기>';")}

${t("-- 2) DBOps에 필요한 최소 권한만 부여")}
${t(
...
${t("GRANT SELECT ON mysql.* TO 'dbops_readonly'@'%';  -- 유저 감사용")}
FLUSH PRIVILEGES;`;

  const passwordCli = `${t("# 3) 강력한 랜덤 비밀번호 생성 (32자)")}
PASSWORD=$(openssl rand -base64 32 | tr -d '=+/' | cut -c1-32)
echo "$PASSWORD"
${t("# → 위 CREATE ROLE / CREATE USER 단계에서 이 값을 사용")}`;

  const secretsManagerCli = `${t(
        ...
        <header className="flex items-center justify-between px-5 py-3 border-b border-zinc-800 flex-shrink-0">
          <div>
            ...
            <div className="text-sm text-zinc-200 mt-0.5">
              {t("DBOps 전용 계정 + Secrets Manager 등록 (프로덕션 권장)")}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-auto px-5 py-4 space-y-5">
          ...
          <section>
            <div className="text-xs text-zinc-400 mb-2">
              ...
              {t("에 등록: bulk Discover가 자동으로 찾아 연결합니다")}
            </div>
          ...
          <section className="border-t border-zinc-800 pt-4">
            <div className="text-[11px] text-zinc-500 leading-relaxed">
              ...
              <br />
              {t("Bulk Discover가 이 시크릿을 발견하면")}{" "}
              <span className="text-emerald-400">convention</span>{" "}
              {t(
                "배지와 함께 자동 연결. 시크릿이 없으면 master 시크릿으로 폴백하면서",
              )}{" "}
              <span className="text-amber-400">master_fallback</span>{" "}
              {t(
