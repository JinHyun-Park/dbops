...
function SidebarItem({ item, active }: { item: NavItem; active: boolean }) {
  // Linear-style nav row: single line, no sub-label. The hint stays as a
  // tooltip-style title so the discoverability is preserved without the
  // visual clutter that made the rail feel "AI-dashboard-default".
  return (
    <Link
      href={item.href}
      title={item.hint}
      className={`group block px-2.5 py-1.5 rounded text-[13px] transition-colors ${
        active
    ...
    <AuthGuard>
      <div className="flex h-screen bg-zinc-950 text-zinc-100">
        ...
        <aside className="hidden md:flex w-56 flex-col border-r border-zinc-800 bg-zinc-950">
          <Link
            href="/"
            className="px-5 py-4 border-b border-zinc-800 hover:bg-zinc-900/50 transition-colors flex items-center gap-2"
          >
            {/* Refined wordmark: single bold "DBOps" with a tiny accent dot
                — replaces the amber/uppercase/Operations two-line block
                that was the loudest "AI-built header" signal. */}
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
            {NAV.map((group) => (
              <div key={group.label}>
                <div className="px-2.5 mb-1.5 text-[10px] tracking-[0.14em] text-zinc-600 font-medium uppercase">
                  {group.label}
                </div>
