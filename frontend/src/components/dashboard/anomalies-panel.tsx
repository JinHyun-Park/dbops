...
function AnomalyDetailModal({
  ...
  const handleAnalyze = () => {
    ...
    const message =
      ...
      // 추정 원인 is a HEDGE, so the English has to hedge too: "Likely cause",
      // never "Cause". See the comment at event-detail-modal.tsx for why the
      // prescribed headings are translated and the clause after each colon is
      // not.
      `1. **${labelIn(
        locale,
        "추정 원인",
        "Likely cause",
      )}**: 메트릭 종류와 패턴을 보고 가장 그럴듯한 설명 ` +
      ...
      `2. **${labelIn(
        ...
        "운영 영향",
        "Operational impact",
      )}**: 지금 사용자나 애플리케이션이 어떤 경험을 하고 있을지.\n` +
      // "Next check step", not "Next check": 단계 is the STEP to take, and the
      // clause below asks for one concrete query or tool to run, not for a
      // checklist item to look at.
      `3. **${labelIn(
        locale,
        "다음 점검 단계",
        "Next check step",
      )}**: 원인을 확정하기 위해 실행할 구체적인 쿼리 1건 또는 MCP 도구 1개. ` +
