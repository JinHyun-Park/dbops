      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
        <div>
          ...
          <div className="text-[11px] text-zinc-500 mt-0.5">
            tables where sequential scans dominate over index scans — candidates for new indexes
          </div>
        </div>
        <select
          value={minRatio}
                ...
                return (
                  <tr key={`${c.schema_name}-${c.table_name}-${i}`} className="hover:bg-zinc-900/40">
                    ...
                    <td
                      ...
                      title={`Sequential scans are ${(ratio * 100).toFixed(1)}% of all scans on this table`}
                    >
                      {(ratio * 100).toFixed(0)}%
                    </td>
                    <td
                      className="px-4 py-2 text-right text-zinc-300 font-mono text-xs tabular-nums"
                      title={`${fmtExact(seqTupRead)} rows read by seq scans — high values indicate wasted IO`}
                    >
