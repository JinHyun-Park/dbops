...
const STATES = [
  ...
  { metric: "conn_idle_in_tx_aborted", label: "idle aborted", color: "#f472b6" },
];
...
export function ConnectionBreakdown({ clusterId, hours = 1 }: { clusterId: string; hours?: number }) {
  ...
  useEffect(() => {
    ...
    fetchClusterSettings(clusterId)
      .then((d) => {
        if (cancelled) return;
        const m = (d.settings || []).find((s: { name: string }) => s.name === "max_connections");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const d = await fetchBatchTimeseries(clusterId, STATES.map((s) => s.metric), hours);
    ...
    <div className="bg-zinc-900/50 border border-zinc-800 p-4 col-span-full">
      ...
      <div className="h-56">
        {loading ? (
          ...
          <div className="text-xs text-zinc-500 flex items-center h-full">no connection data yet</div>
        ) : (
            ...
            <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              {maxConn !== null && (
                <ReferenceLine
                  ...
                  stroke="#ef4444"
                  strokeDasharray="4 4"
                  label={{ value: `max ${maxConn}`, fill: "#ef4444", fontSize: 10, position: "right" }}
                />
              )}
              {STATES.map((s) => (
                <Area
