...
const STEPS: Step[] = [
  {
    ...
    body: (
      <>
        DBOps는 Amazon Aurora MySQL / PostgreSQL을 플릿 단위로 운영하는 DBA를 위해 만든 콘솔입니다.
        모든 패널이 에이전트와 연결돼 있어, 이상 징후·이벤트·발견 항목을 클릭하면{" "}
        <span className="text-sky-300">한 번에 원인 분석과 조치 제안</span>을 받을 수 있습니다.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <span className="font-mono text-amber-300">Clusters</span> 페이지에서 시작하세요. 한두 개라면
        수동 등록 폼을, 플릿 규모라면{" "}
        <span className="font-mono text-sky-300">🔎 Discover clusters</span> 버튼을 사용하면
        됩니다 — DBOps가 계정 내(또는 크로스 어카운트 롤 경유) 모든 Aurora 클러스터를 나열하고,
        체크한 것만 등록합니다.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        ETL이 5분 주기로 메트릭, 테이블 통계, 락, 점검 결과를 수집합니다. 첫 사이클 전까지
        대시보드는 <span className="text-zinc-400 italic">no data yet</span>으로 표시됩니다. 이
        시간 동안 <span className="font-mono text-amber-300">Alerts</span>에서 Slack / PagerDuty
        구독자를 등록해두면, 임계치를 초과하는 즉시 알림을 받을 수 있습니다.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        <span className="font-mono text-amber-300">Chat</span>을 열고 다음과 같이 물어보세요:{" "}
        ...
        에이전트가 MCP 툴로 Performance Insights를 보고, EXPLAIN을 돌리고, 조치를 제안합니다. 기본은
        읽기 전용이며, 변경 작업은 Approval Center를 통해서만 적용됩니다.
      </>
    ),
