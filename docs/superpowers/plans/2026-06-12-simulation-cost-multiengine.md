...
**Goal:** Make the Simulator behave correctly for DocumentDB/DynamoDB: gate the Aurora-only Simulator (page + 6 MCP tools) off cleanly for NoSQL. (DocDB cost rightsizing, originally Part 2, is DEFERRED; see spec: DocDB lacks instance_class and uses a different CPU metric_type, so it needs collector plumbing first.)
